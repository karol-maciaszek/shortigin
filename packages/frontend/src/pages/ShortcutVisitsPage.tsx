...
import { useGetShortcutVisitsSubscription } from '../generated/urql.user'
...
export const ShortcutVisitsPage = () => {
  ...
  const [{ data: getShortcutVisitsData, fetching: getShortcutVisitsFetching }] =
    useGetShortcutVisitsSubscription({
      variables: parsedId ? { where: { shortcutId: { _eq: parsedId } } } : {},
    })
  ...
  return (
    <div>
      <div className={`h-[calc(100vh-${parsedId ? 12 : 6}rem)] w-full`}>
        {parsedId && (
          <p className="mt-3 mb-6 flex items-center gap-1">
            <button className="no-underline" onClick={() => navigate(`/app/shortcuts/${parsedId}`)}>
              &lt; Back
            </button>
          </p>
        )}
        <div className="flex justify-center overflow-y-auto h-full">
          <table className="table-auto h-fit w-full">
            <thead>
              <tr>
                <th className="p-3 pl-6 text-left border-b border-b-neoncyan text-coolgray">
                  Date
                </th>

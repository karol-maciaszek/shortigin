...
export function shortcutInsertWebhook({
  sdk,
  hashids,
}: {
  sdk: Pick<ReturnType<typeof getSdk>, 'UpdateShortcutSlug'>
  hashids: Hashids
}) {
  return async (req: Request, res: Response<{ message: string }>) => {
    ...
    await sdk.UpdateShortcutSlug({
      id: payload.event.data.new.id,
      slug: hashids.encode([payload.event.data.new.id]),
    })
    res.send({ message: 'ok' })
  }
}
